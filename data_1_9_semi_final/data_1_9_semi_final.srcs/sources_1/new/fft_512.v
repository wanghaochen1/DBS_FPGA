...
always @ (posedge clk or negedge rst_n) begin
    if(!rst_n) begin  //进行初始�?
        ...
        if(count == 10'd511) begin  //�?后一位的数据输入
            ...
            fft_s_data_tdata <= {mult_result,32'd0};  // 虚部�?0，实部为mult_result 
            count <= 10'd0;
...
///    // 实例化ILA模块,进行上板测试 //
    ...
    reg signed [41:0] fft_re_out;
    reg signed [41:0] fft_im_out;
// ila_fft your_instance_name (
// 	.clk(clk), // input wire clk


// 	.probe0(fft_m_data_tuser), // input wire [15:0]  probe0  
// 	.probe1(fft_m_data_tvalid), // input wire [0:0]  probe1 
// 	.probe2(fft_re_out), // input wire [41:0]  probe2 
// 	.probe3(fft_im_out) // input wire [41:0]  probe3
// );
...
FFT_512 u_fft(
    ...
    .s_axis_config_tdata(8'd1),                                // ip核设置参数内容，�?1时做FFT运算，为0时做IFFT运算（input�?
    ...
    .s_axis_data_tready(fft_s_data_tready),                    // 表示从设备已经准备好接收�?次数据传输（output，从->主），当tvalid和tready同时为高时，启动数据传输
    .s_axis_data_tlast(fft_s_data_tlast),                      // 主设备向从设备发送传输结束信号（input，主->从，拉高为结束）
    //作为发�?�频谱数据时是主设备
    .m_axis_data_tdata(fft_m_data_tdata),                      // FFT输出的频谱数据，[89:48]对应的是虚部数据，[41:0]对应的是实部数据(output，主->�?)�?
    .m_axis_data_tuser(fft_m_data_tuser),                      // 输出频谱的索�?(output，主->�?)，该�?*fs/N即为对应频点�?
    .m_axis_data_tvalid(fft_m_data_tvalid),                    // 表示主设备正在驱动一个有效的传输（output，主->从）
    .m_axis_data_tready(fft_m_data_tready),                    // 表示从设备已经准备好接收�?次数据传输（input，从->主），当tvalid和tready同时为高时，启动数据传输
